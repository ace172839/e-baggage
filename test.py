...
async def main(page: ft.Page):
    ...
    request_permission = await gl.request_permission_async(wait_timeout=60)
    get_permission_status = await gl.get_permission_status_async()
    current_position = await gl.get_current_position_async()
    last_known_position = await gl.get_last_known_position_async()

    print(f"gl.request_permission: {request_permission}, \n\
          gl.get_permission_status: {get_permission_status}, \n\
          gl.get_current_position: {current_position}, \n\
          gl.get_last_known_position: {last_known_position} \n")
